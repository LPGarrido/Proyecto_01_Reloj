...
PSECT udata_bank0
    ...
    UNIDADES_SEG: DS 1 ; Decenas del valor
    DECENAS_SEG: DS 1 ; Unidades del valor
    UNIDADES_MIN: DS 1 ; Decenas del valor
    DECENAS_MIN: DS 1 ; Unidades del valor
    UNIDADES_HORA: DS 1 ; Decenas del valor
    DECENAS_HORA: DS 1 ; Unidades del valor
    HORAS: DS 1
    valor: DS 1
    BANDERAS: DS 1 ; Indica que display hay que encender
    NUMEROS: DS 4 ; Nibbles del valor (alto y bajo)
...
ORG 200h ; posición 100h para el codigo
;------------- CONFIGURACION ------------
MAIN:
...
CONFIG_TMR2:
    ...
    MOVLW 5 ; Valor para interrupciones cada 500ms
...
CHECK_UNI_SEG:
    MOVF UNIDADES_SEG,W ;W=UNIDADES
    SUBLW 10 ;W-10 = UNIDADES - 10
    BTFSS STATUS,2 ;Si Z=0, regresa; si Z=1 funcion
    RETURN ;regresa
    INCF DECENAS_SEG ;incrementar decenas
    CLRF UNIDADES_SEG ;limpiar unidades
    RETURN

CHECK_DEC_SEG:
    MOVF DECENAS_SEG,W ;W=DECENAS
    SUBLW 6 ;W = DECENAS - 6
    BTFSS STATUS,2 ;Si Z=0, regresa; si Z=1 funcion
    RETURN ;regresa
    INCF UNIDADES_MIN
    CLRF DECENAS_SEG ;limpiar decenas
    RETURN ;regresa

CHECK_UNI_MIN:
    MOVF UNIDADES_MIN,W ;W=UNIDADES
    SUBLW 10 ;W-10 = UNIDADES - 10
    BTFSS STATUS,2 ;Si Z=0, regresa; si Z=1 funcion
    RETURN ;regresa
    INCF DECENAS_MIN ;incrementar decenas
    CLRF UNIDADES_MIN ;limpiar unidades
    RETURN

CHECK_DEC_MIN:
    MOVF DECENAS_MIN,W ;W=DECENAS
    SUBLW 6 ;W = DECENAS - 6
    BTFSS STATUS,2 ;Si Z=0, regresa; si Z=1 funcion
    RETURN ;regresa
    INCF HORAS
    CLRF DECENAS_MIN ;limpiar decenas
    RETURN ;regresa

OBTENER_HORA: ; Ejemplo: HORA = 23
    CHECK_DEC_HORA:
    MOVLW 10 ; W=10
    SUBWF HORAS,W ; HORAS - 10
    BTFSS STATUS,0 ; Si 10>HORAS, C=0 CHECK_UNI, ; 10=<HORAS, C=1 funcion
    GOTO CHECK_UNI_HORA ;
    MOVWF HORAS ;HORAS = HORAS - 10
    INCF DECENAS_HORA ;decenas = decenas+1
    GOTO CHECK_DEC_HORA

    CHECK_UNI_HORA:
    MOVLW 1 ; W=1
    SUBWF HORAS,W ; HORAS - 1
    BTFSS STATUS,0 ; Si 1>HORAS, C=0, ; 1=<HORAS, C=1
    RETURN ; Retorna
    MOVWF HORAS ;valor = valor - 1
...
SET_DISPLAY:
    MOVF UNIDADES_MIN, W ; Movemos centenas a W
    CALL TABLA_7SEG ; Buscamos valor a cargar en PORTC
    MOVWF DISPLAY+3 ; Guardamos en display

    MOVF DECENAS_MIN, W ; Movemos decenas a W
    CALL TABLA_7SEG ; Buscamos valor a cargar en PORTC
    MOVWF DISPLAY+2 ; Guardamos en display

    MOVF UNIDADES_HORA, W ; Movemos unidades a W
    CALL TABLA_7SEG ; Buscamos valor a cargar en PORTC
    MOVWF DISPLAY+1 ; Guardamos en display

    MOVF DECENAS_HORA, W ; Movemos unidades a W
    CALL TABLA_7SEG ; Buscamos valor a cargar en PORTC
